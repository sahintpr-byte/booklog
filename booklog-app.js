...
async function searchBooks(){
  ...
  try{
    const r=await fetch('https://www.googleapis.com/books/v1/volumes?q='+encodeURIComponent(q)+'&maxResults=20&langRestrict=');
...
function openModal(item){
  ...
  document.getElementById('modalAuthor').textContent=(info.authors||[]).join(', ')||'';
  document.getElementById('modalMeta').textContent=(info.publishedDate?info.publishedDate.slice(0,4):'')+(info.pageCount?' · '+info.pageCount+' sayfa':'')+(info.categories?' · '+(info.categories[0]||''):'');
...
async function saveBook(){
  ...
  const {error}=await sb.from('book_lists').upsert({
    user_id:currentUser.id,book_id:currentBook.id,list_id:listId,
    book_title:info.title||'',book_author:(info.authors||[]).join(', '),
...
async function importList(event){
  ...
  for(const row of rows){
    ...
    try{
      const r=await fetch('https://www.googleapis.com/books/v1/volumes?q='+encodeURIComponent(bookTitle)+'&maxResults=1');
      const d=await r.json();const item=d.items&&d.items[0];
      if(!item){skipped++;continue;}
      const info=item.volumeInfo||{};const cover=info.imageLinks?.thumbnail?.replace('http:','https:')||'';
      await sb.from('book_lists').upsert({
        user_id:currentUser.id,book_id:item.id,list_id:activeListId,
        book_title:info.title||bookTitle,book_author:(info.authors||[]).join(', '),
        book_image:cover,published_year:info.publishedDate?info.publishedDate.slice(0,4):'',
        categories:info.categories||[],page_count:info.pageCount||0,
